...
class ProvidersScreenDoneTasks2 extends StatefulWidget {
  const ProvidersScreenDoneTasks2({super.key});

  @override
  _ProvidersWithRatingsScreenState createState() => _ProvidersWithRatingsScreenState();
}

class _ProvidersWithRatingsScreenState extends State<ProvidersScreenDoneTasks2> {
  String _searchQuery = '';
  ...
  return Scaffold(
    ...
    body: Column(
      children: [
        ...
        // Display the number of providers in Arabic text
        StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            int providerCount = snapshot.data!.docs.length;
            return Padding(
              padding: const EdgeInsets.all(16.0),
              child: Text(
                'عدد مقدمي الخدمات: ${providerCount.toString()}',
                style: GoogleFonts.cairo(
                  fontSize: 18,
        ...
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              // Fetch task counts for all providers
              return FutureBuilder<List<Map<String, int>>>(
                future: Future.wait(
                  filteredProviders.map((provider) async {
                    String email = provider['email'] ?? '';
                    return _fetchTaskCounts(email);
                  }).toList(),
                ),
                builder: (context, taskCountsSnapshot) {
                  if (taskCountsSnapshot.connectionState == ConnectionState.waiting) {
                    return const Center(child: CircularProgressIndicator());
                  }

                  if (!taskCountsSnapshot.hasData) {
                    return Center(
                      child: Text(
                        'فشل في جلب بيانات المهام',
                        style: GoogleFonts.cairo(),
                      ),
                    );
                  }

                  // Map task counts to providers
                  // Map task counts to providers
List<Map<String, int>> taskCounts = taskCountsSnapshot.data!;
Map<String, int> providerTaskCounts = {};
for (int i = 0; i < filteredProviders.length; i++) {
  String email = filteredProviders[i]['email'] ?? '';
  providerTaskCounts[email] = taskCounts[i]['done'] ?? 0;
}

// Sort providers by completed tasks (done) in ascending order
filteredProviders.sort((a, b) {
  int aDone = providerTaskCounts[a['email'] ?? ''] ?? 0;
  int bDone = providerTaskCounts[b['email'] ?? ''] ?? 0;
  return aDone.compareTo(bDone); // Sort in ascending order
});
