...
class ProvidersScreenAlpha2 extends StatefulWidget {

  const ProvidersScreenAlpha2({super.key});

  @override
  _ProvidersScreenState createState() => _ProvidersScreenState();
}

class _ProvidersScreenState extends State<ProvidersScreenAlpha2> {
  String _searchQuery = '';
  ...
  child: StreamBuilder<QuerySnapshot>(
    ...
    builder: (context, snapshot) {
      ...
      // Fetch and sort providers from Z to A
      var sortedProviders = snapshot.data!.docs.toList()
        ..sort((a, b) {
          String nameA = (a['name'] ?? '').toLowerCase();
          String nameB = (b['name'] ?? '').toLowerCase();
          return nameB.compareTo(nameA); // Reverse the comparison for Z to A
        });

      // Filter providers based on the search query
      var filteredProviders = sortedProviders.where((provider) {
