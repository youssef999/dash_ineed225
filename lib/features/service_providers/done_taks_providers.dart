...
class ProvidersScreenDoneTasks extends StatefulWidget {
  const ProvidersScreenDoneTasks({super.key});

  @override
  _ProvidersWithRatingsScreenState createState() => _ProvidersWithRatingsScreenState();
}

class _ProvidersWithRatingsScreenState extends State<ProvidersScreenDoneTasks> {
  String _searchQuery = '';
            ...
            builder: (context, snapshot) {
              ...
              return FutureBuilder<List<Map<String, int>>>(
                ...
                builder: (context, taskCountsSnapshot) {
                  ...
                  // Sort providers by completed tasks (done)
                  filteredProviders.sort((a, b) {
                    int aDone = providerTaskCounts[a['email'] ?? ''] ?? 0;
                    int bDone = providerTaskCounts[b['email'] ?? ''] ?? 0;
                    return bDone.compareTo(aDone); // Sort in descending order
                  });
