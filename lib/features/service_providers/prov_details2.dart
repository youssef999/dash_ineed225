...
class ProviderDetails20 extends StatefulWidget {
  final Map<String, dynamic> provider;

  ProviderDetails20({Key? key, required this.provider}) : super(key: key);

  @override
  State<ProviderDetails20> createState() => _ProviderDetailsState();
}

class _ProviderDetailsState extends State<ProviderDetails20> {
  int numTasks = 0;
  int numTasksDone = 0;
  ...
  Future<void> _fetchProviderStats() async {
    ...
    try {
      ...
      setState(() {
        ...
        numTasksAccepted = tasks.where((task) => task['status'] == 'accepted').length;
        numTasksCancelled = tasks.where((task) => task['status'] == 'canceled').length;
        numTasksPending = tasks.where((task) => task['status'] == 'pending').length;
      });
      ...
      setState(() {
        ...
        averageRating = comments.isNotEmpty
            ? comments.map((c) => c['rate'] ?? 0).reduce((a, b) => a + b) / comments.length
            : 0.0;
  ...
  Future<void> _fetchBuyServiceStats(String email) async {
    try {
      ...
      setState(() {
        ...
        numBuyServiceDone = buyServices.where((service) => service['status'] == 'done').length;
        numBuyServicePending = buyServices.where((service) => service['status'] == 'pending').length;
        numBuyServiceAccepted = buyServices.where((service) => service['status'] == 'accepted').length;
        numBuyServiceCanceled = buyServices.where((service) => service['status'] == 'canceled').length;
      });
    } catch (e) {
