...
class OffersSubCatView extends StatefulWidget {
  final String subCats;

  const OffersSubCatView({super.key, required this.subCats});

  @override
  State<OffersSubCatView> createState() => _OffersSubCatViewState();
}

class _OffersSubCatViewState extends State<OffersSubCatView> {
  ...
  List<QueryDocumentSnapshot> _filterOffers(List<QueryDocumentSnapshot> offers) {
    final subCatsList = widget.subCats.split(',').map((e) => e.trim()).toList();
    return offers.where((offer) {
      final subCat = offer['sub_cat'] as String? ?? '';
